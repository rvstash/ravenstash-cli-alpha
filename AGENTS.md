...
- The CLI manages Ravenstash credentials and delegates install flows to native toolchains where appropriate.
- CLI code lives under `rvn/`, with registry-specific behavior under `rvn/registries/` and command wiring under `rvn/commands/`.

## Working rules

- Keep user credentials in the established config/keyring flow; do not add ad hoc token storage.
