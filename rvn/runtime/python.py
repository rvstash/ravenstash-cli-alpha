"""Install Python runtimes using python-build-standalone (same source as uv).

Downloads a self-contained CPython build from:
  https://github.com/astral-sh/python-build-standalone/releases

Installs to: ~/.rvn/runtimes/python/<full_version>/
"""

from __future__ import annotations

import re
import sys
import tempfile
from pathlib import Path
from typing import Any

import httpx

from ._install import (
    RUNTIMES_DIR,
    check_linux_x64,
    download,
    extract,
    write_env_file,
    write_shim,
)


_REPO = "astral-sh/python-build-standalone"
_GH_RELEASES = f"https://api.github.com/repos/{_REPO}/releases"

# Map (system_arch → pbs_arch)
_ARCH_MAP = {"x64": "x86_64", "aarch64": "aarch64"}


def _gh_headers() -> dict[str, str]:
    return {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
    }


def _find_asset(version_prefix: str, arch: str) -> tuple[str, str]:
    """Return (full_version, download_url) for the best matching PBS asset."""
    suffix = f"{arch}-unknown-linux-gnu-install_only.tar.gz"
    pattern = re.compile(r"cpython-(\d+\.\d+\.\d+)\+\d+-" + re.escape(suffix))
    # Search up to 5 pages of releases
    for page in range(1, 6):
        with httpx.Client(timeout=30.0) as hx:
            resp = hx.get(
                _GH_RELEASES,
                headers=_gh_headers(),
                params={"per_page": "10", "page": str(page)},
            )
            resp.raise_for_status()
            releases: list[dict[str, Any]] = resp.json()
        if not releases:
            break
        for release in releases:
            for asset in release.get("assets", []):
                name: str = asset["name"]
                m = pattern.match(name)
                if not m:
                    continue
                full_ver = m.group(1)
                # Match if full_ver starts with the requested prefix
                vp = version_prefix.rstrip(".")
                if full_ver == vp or full_ver.startswith(vp + "."):
                    return full_ver, asset["browser_download_url"]
    print(
        f"No Python {version_prefix} build found for linux/{arch}.\n"
        f"Check: https://github.com/{_REPO}/releases",
        file=sys.stderr,
    )
    sys.exit(1)


def install(version: str) -> Path:
    """Download and install Python *version* to ``~/.rvn/runtimes/python/``.

    *version* may be ``"3.12"``, ``"3.12.1"``, etc.
    Returns the installation directory.
    """
    _, arch_raw = check_linux_x64()
    arch = _ARCH_MAP.get(arch_raw, arch_raw)

    print(f"Resolving Python {version} (python-build-standalone) ...", flush=True)
    full_ver, url = _find_asset(version, arch)

    dest = RUNTIMES_DIR / "python" / full_ver
    if dest.exists():
        print(f"Python {full_ver} already installed at {dest}")
        return dest

    with tempfile.TemporaryDirectory() as tmp:
        archive = Path(tmp) / url.split("/")[-1].split("?")[0]
        download(url, archive)
        extract(archive, dest)

    # Write shims for python3 / python3.x
    bin_dir = dest / "bin"
    python3 = bin_dir / "python3"
    if not python3.exists():
        candidates = sorted(bin_dir.glob("python3.*"))
        if candidates:
            python3 = candidates[0]
    if python3.exists():
        write_shim("python3", python3)
        minor = ".".join(full_ver.split(".")[:2])  # e.g. "3.14"
        write_shim(f"python{minor}", python3)

    write_env_file()
    print(f"✓ Python {full_ver} installed at {dest}")
    print("  Add to PATH:  source ~/.rvn/env")
    return dest


def list_installed() -> list[tuple[str, Path]]:
    """Return [(version, path), ...] for all rvn-managed Python installations."""
    base = RUNTIMES_DIR / "python"
    if not base.exists():
        return []
    return sorted(
        [(p.name, p) for p in base.iterdir() if p.is_dir()],
        key=lambda t: t[0],
    )


def find(version_prefix: str) -> Path | None:
    """Return the path of the first installed Python matching *version_prefix*."""
    vp = version_prefix.rstrip(".")
    if not vp:
        installed = list_installed()
        return installed[-1][1] if installed else None
    for ver, path in list_installed():
        if ver == vp or ver.startswith(vp + "."):
            return path
    return None


def python_bin(version_prefix: str) -> Path | None:
    """Return the ``python3`` binary path for *version_prefix*, or None."""
    base = find(version_prefix)
    if base is None:
        return None
    for name in (f"python{'.'.join(version_prefix.split('.')[:2])}", "python3", "python"):
        p = base / "bin" / name
        if p.exists():
            return p
    return None
