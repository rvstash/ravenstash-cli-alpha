...
def _select_profile_interactive(cfg: cfg_mod.RvnConfig) -> str:
    ...
    if not sys.stdin.isatty() or not output.console.is_terminal:
        output.fatal("Cannot open profile selector. Use `rvn auth profile switch <name>`.")
...
def whoami(
    ...
    """Print the active profile and customer from local metadata."""
    cfg = cfg_mod.load()
    profile_name = _target_profile(profile, cfg)
    p = cfg.active_profile(profile_name)
    output.kv(
...
def profile_list() -> None:
    ...
    cfg = cfg_mod.load()
    if not cfg.profiles:
        output.info("No profiles configured. Run `rvn auth login` first.")
        return
...
def profile_rename(
    ...
    output.info(
        f"Run `rvn auth login --profile {new}` to store credentials for the renamed profile."
    )
