...
_RVN_URL_KINDS: dict[NativeTool, tuple[RegistryKind, ...]] = {
    "pip": ("pypi",),
    "uv": ("pypi",),
    "twine": ("pypi",),
    "npm": ("npm",),
...
def normalize_policy(value: str) -> ConfigPolicy:
    ...
    if policy not in POLICIES:
        ...
        output.fatal(f"Unknown --rvn-native-config '{value}'. Use one of: {joined}.")
...
def run(tool: NativeTool, argv: list[str], options: NativeOptions) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rvn-native-") as temp_dir:
...
def _customer_public_id(profile: str | None, explicit_customer_pid: str | None) -> str:
    ...
    if not profile_cfg.customer_public_id:
        output.fatal(
            ...
            "Run `rvn auth login` again, pass --rvn-customer-pid, or pass "
            "--rvn-repo <customer-public-id>/<repository-name>."
        )
...
def _resolve_route(kind: RegistryKind, options: NativeOptions) -> RegistryRoute:
    cfg = cfg_mod.load()
    repo_ref = options.repo or cfg.registry_defaults(kind).default_repo
    if not repo_ref:
        output.fatal(
            f"No {kind} package repository selected. Pass --rvn-repo or run "
            f"`rvn pkg repo set-default {kind} <repository-name>`."
        )
...
def _detected_ravenstash_urls(
    ...
    urls: list[str] = []
    seen: set[str] = set()
    allowed_kinds = _RVN_URL_KINDS[tool]
...
def _ensure_maven_profile(root: ET.Element, route: RegistryRoute) -> None:
    ...
    profile = _find_child_with_text(profiles, "profile", "id", "rvn")
    if profile is None:
        profile = ET.SubElement(profiles, "profile")
        ET.SubElement(profile, "id").text = "rvn"
    ...
    if _find_child_text(active_profiles, "activeProfile", "rvn") is None:
        ET.SubElement(active_profiles, "activeProfile").text = "rvn"


def _upsert_maven_repository(parent: ET.Element, tag: str, url: str) -> None:
    repo = _find_child_with_text(parent, tag, "id", "rvn-private")
    if repo is None:
        repo = ET.SubElement(parent, tag)
        ET.SubElement(repo, "id").text = "rvn-private"
