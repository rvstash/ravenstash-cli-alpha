...
def _options(
    ...
    return runner.OciOptions(profile=profile, repo=repo, customer_id=customer_id, kind=kind)
...
def _run(
    ...
    runner.run(tool, list(ctx.args), _options(profile, repo, customer_id, kind))
...
def docker(
    ...
    _run("docker", ctx, rvs_profile, rvs_repo, rvs_customer_id, "container")
...
def helm(
    ...
    _run("helm", ctx, rvs_profile, rvs_repo, rvs_customer_id, "helm")
...
def oras(
    ...
    _run("oras", ctx, rvs_profile, rvs_repo, rvs_customer_id, rvs_kind)
...
def oci_reference(
    ...
    typer.echo(
        runner.reference(
            kind=kind,
            options=_options(profile, repo, customer_id, kind),
            oci_path=oci_path,
