...
def test_pypi_publish_reads_wheel_metadata_and_posts_legacy_upload(
    ...
    result = pypi_reg.publish(
        upload_url="https://api.example/pypi/r/repo",
        token="secret-token",
        files=[wheel],
    )
    ...
    assert calls[0]["url"] == "https://api.example/pypi/r/repo"
...
def test_pypi_publish_reports_http_failures(monkeypatch, tmp_path: Path) -> None:
    ...
    result = pypi_reg.publish("https://api.example/pypi/r/repo", "token", [sdist])
...
def test_npm_publish_builds_scoped_publish_body(monkeypatch, tmp_path: Path) -> None:
    ...
    result = npm_reg.publish(
        registry_url="https://api.example/npm/r/repo/",
        token="secret-token",
    ...
    assert calls[0]["url"] == "https://api.example/npm/r/repo/@scope/demo"
    ...
    assert body["versions"]["1.2.3"]["dist"]["tarball"] == (
        "https://api.example/npm/r/repo/@scope/demo/-/scope-demo-1.2.3.tgz"
    )
...
def test_maven_publish_uploads_artifact_and_checksum_sidecars(
    ...
    result = maven_reg.publish(
        upload_url="https://api.example/maven/r/repo",
        token="secret-token",
    ...
    assert [call["url"] for call in calls] == [
        "https://api.example/maven/r/repo/com/example/demo/1.0.0/demo-1.0.0.jar",
        "https://api.example/maven/r/repo/com/example/demo/1.0.0/demo-1.0.0.jar.md5",
        "https://api.example/maven/r/repo/com/example/demo/1.0.0/demo-1.0.0.jar.sha1",
    ]
...
def test_maven_settings_xml_contains_repo_token_and_server_id() -> None:
    xml = maven_reg._build_settings_xml(
        "https://api.example/maven/r/repo/",
        "secret-token",
    ...
    assert "<url>https://api.example/maven/r/repo/</url>" in xml
